...
GO
ALTER PROCEDURE [projection].[test nulls_format_correctly]
AS
BEGIN
  	-----------------------------------------------------------------------------------------------------------------
	-- Arrange:
	...
	(
		...
		34827897, 
		1, 
		N'[{"key":[{"CustId":"_Test3fffrtyhrt"}],"detail":[{"TotalStep":{"from":14,"to":15},"S15":{"from":null,"to":"FileManagement"}}]}]' 
	);
	...
	-----------------------------------------------------------------------------------------------------------------
	 ...
	 DECLARE @expectedJSON nvarchar(MAX) = N'[{"key":[{"CustId":"_Test3fffrtyhrt"}],"detail":[{"TotalStep":{"from":14,"to":15},"S15":{"from":null,"to":"FileManagement"}}]}]';
