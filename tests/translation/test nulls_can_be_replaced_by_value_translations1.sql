...
CREATE OR ALTER PROCEDURE [translation].[test nulls_can_be_replaced_by_value_translations]
AS
BEGIN
  	-----------------------------------------------------------------------------------------------------------------
	-- Arrange:
	...
	-- e.g., that null (well, 'null') can be replaced by, say, 'missing'
	...
	(
		...
		34827897, 
		1, 
		N'[{"key":[{"bigid":10060000}],"detail":[{"some_data":{"from":"test","to":null}}]}]' 
	);
	...
	VALUES	(
		...
		N'null', -- key_value - sysname
		N'-DEFAULT-' -- translation_value - sysname
	);
	...
	-----------------------------------------------------------------------------------------------------------------
	 ...
	 DECLARE @expectedJSON nvarchar(MAX) = N'[{"key":[{"bigid":10060000}],"detail":[{"some_data":{"from":"test","to":"-DEFAULT-"}}]}]';
