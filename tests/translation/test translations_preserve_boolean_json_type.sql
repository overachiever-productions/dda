...
BEGIN
	...
	(
		...
		34827897, 
		1, 
		N'[{"key":[{"bigid":10060000}],"detail":[{"some_data":{"from":false,"to":true}}]}]' 
	);
	...
	VALUES	(
		...
		N'false', -- key_value - sysname
		N'NEGATIVE' -- translation_value - sysname
	);
	...
	-----------------------------------------------------------------------------------------------------------------
	 ...
	 DECLARE @expectedJSON nvarchar(MAX) = N'[{"key":[{"bigid":10060000}],"detail":[{"some_data":{"from":"NEGATIVE","to":true}}]}]';
