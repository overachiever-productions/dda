...
CREATE OR ALTER PROCEDURE [translation].[test translations_preserve_null_data_type]
AS
BEGIN
  	-----------------------------------------------------------------------------------------------------------------
	-- Arrange:
	...
	VALUES	(
		...
		N'test', -- key_value - sysname
		N'T_E_S_T' -- translation_value - sysname
	);
	...
	-----------------------------------------------------------------------------------------------------------------
	 ...
	 DECLARE @expectedJSON nvarchar(MAX) = N'[{"key":[{"bigid":10060000}],"detail":[{"some_data":{"from":"T_E_S_T","to":null}}]}]';
