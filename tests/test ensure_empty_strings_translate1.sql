...
ALTER PROCEDURE [transformations].[test ensure_empty_strings_translate]
AS
BEGIN
  	-----------------------------------------------------------------------------------------------------------------
	-- Arrange:
	...
	VALUES	(
		...
		N'', -- key_value - sysname
		N'<EMPTY>' -- translation_value - sysname
	);
	...
	-----------------------------------------------------------------------------------------------------------------
	 ...
	 DECLARE @expectedJSON nvarchar(MAX) = N'[{"key":[{"CustId":"_Test3fffrtyhrt"}],"detail":[{"TotalStep":{"from":14,"to":15},"S15":{"from":"<EMPTY>","to":"FileManagement"}}]}]';
